...
- **Custom domain `riotriot.com.br` — ✅ LIVE** (verified serving with SSL: site, `/rider`, and the og share image all return 200; nameservers on Cloudflare `javier`/`luciana.ns.cloudflare.com`; DNSSEC off; `og:image`/`og:url` point to the domain). Remaining optional: **`www.riotriot.com.br` redirect** — `www` is NOT configured (doesn't resolve); add a `www → riotriot.com.br` redirect (Cloudflare Redirect Rule + a proxied DNS record for `www`) if wanted. Share preview: it's a fresh URL, so the first WhatsApp/FB share scrapes the correct branded card automatically.
